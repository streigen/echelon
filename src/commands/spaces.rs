...
pub async fn get_spaces(state: ClientState) -> Result<Vec<SpaceRoom>, String> {
...
pub async fn get_all_spaces_with_trees(
    ...
    let joined_spaces =
        super::with_active_client(state.clone(), |client_handler| client_handler.get_client().joined_space_rooms())
            .await?;
